...
export async function runTemplateCompose(ir: PromptIR): Promise<PromptIR> {
  try {
    ...
    if (ir.yafa && ir.advancedTechniques) {
      ...
      composedPrompt = advancedResult.prompt
    } else {
      // Fallback to standard LLM generation
      composedPrompt = await promptGenerator.generatePrompt(ir, {
        temperature: ir.yafa ? 0.8 : 0.7, // Higher creativity for YAFA mode
        useYAFAMode: ir.yafa,
      })
    }

    // Create IR with initial prompt for quality gate processing
    const irWithPrompt = { ...ir, composedPrompt }

    // Apply Quality Gate - iterative refinement until quality threshold is met
    console.log('🎯 Applying quality gate enforcement...')
    const qualityResult = await qualityGateAgent.enforceQualityThreshold(irWithPrompt)
    
    console.log(`🏆 Final quality: ${qualityResult.qualityScore.overall}% after ${qualityResult.iterations} iteration(s)`)
    if (qualityResult.improvementLog.length > 0) {
      console.log('📝 Improvement log:', qualityResult.improvementLog)
    }
    
    return { 
      ...ir, 
      composedPrompt: qualityResult.finalPrompt,
      qualityMetadata: {
        finalScore: qualityResult.qualityScore,
        iterations: qualityResult.iterations,
        improvementLog: qualityResult.improvementLog
      }
    }
  } catch (error) {
